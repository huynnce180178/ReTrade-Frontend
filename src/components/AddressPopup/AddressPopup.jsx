      ...
      <div className="bg-surface-container-lowest w-full max-w-lg rounded-xl shadow-2xl overflow-hidden transform transition-transform duration-300">
        ...
        <div className="p-6 border-b border-outline-variant flex justify-between items-center">
          <h3 className="font-headline-sm text-headline-sm text-secondary">
            {isAddingNew ? t('account.add_address') : t('checkout.select_address')}
          </h3>
          <button onClick={onClose} className="text-on-surface-variant hover:text-secondary transition-colors" aria-label={t('common.close')}>
        ...
        <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto font-body-md">
          {!isAddingNew ? (
            <>
              {addresses.length === 0 ? (
                <p className="text-on-surface-variant text-center">{t('common.no_data')}</p>
              ) : (
                      ...
                      >
                        ...
                        <div className="flex-grow">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {addr.isDefault && <span className="bg-secondary text-on-secondary text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-widest">{t('account.default_address')}</span>}
                          </div>
              ...
              >
                <span className="material-symbols-outlined">add_circle</span> {t('account.add_address')}
              </button>
            </>
          ) : (
            <form id="new-address-form" onSubmit={handleAddNew} className="space-y-4">
              <div className="flex flex-col gap-1">
                <label className="font-label-caps text-on-surface-variant uppercase">{t('account.full_name')}</label>
              ...
              <div className="flex flex-col gap-1">
                <label className="font-label-caps text-on-surface-variant uppercase">{t('auth.phone')}</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1">
                  <label className="font-label-caps text-on-surface-variant uppercase">{t('account.province')}</label>
                  <select required value={provinceId} onChange={handleProvinceChange} className="border border-outline-variant rounded-md py-2 px-3 text-secondary focus:border-secondary focus:ring-1 focus:ring-secondary focus:outline-none transition-all bg-transparent text-sm">
                    <option value="">{t('common.select')} {t('account.province')}</option>
                    {provinces.map((p) => (
                ...
                <div className="flex flex-col gap-1">
                  <label className="font-label-caps text-on-surface-variant uppercase">{t('account.district')}</label>
                  <select required disabled={!provinceId} value={districtId} onChange={handleDistrictChange} className="border border-outline-variant rounded-md py-2 px-3 text-secondary focus:border-secondary focus:ring-1 focus:ring-secondary focus:outline-none transition-all disabled:bg-surface-container-low text-sm bg-transparent">
                    <option value="">{t('common.select')} {t('account.district')}</option>
                    {districts.map((d) => (
              ...
              <div className="flex flex-col gap-1">
                <label className="font-label-caps text-on-surface-variant uppercase">{t('account.ward')}</label>
                <select required disabled={!districtId} value={wardCode} onChange={(e) => setWardCode(e.target.value)} className="border border-outline-variant rounded-md py-2 px-3 text-secondary focus:border-secondary focus:ring-1 focus:ring-secondary focus:outline-none transition-all disabled:bg-surface-container-low text-sm bg-transparent">
                  <option value="">{t('common.select')} {t('account.ward')}</option>
                  {wards.map((w) => (
              ...
              <div className="flex flex-col gap-1">
                <label className="font-label-caps text-on-surface-variant uppercase">{t('account.detail_address')}</label>
                <input required type="text" value={street} onChange={(e) => setStreet(e.target.value)} className="border border-outline-variant rounded-md py-2 px-3 text-secondary focus:border-secondary focus:ring-1 focus:ring-secondary focus:outline-none transition-all bg-transparent" placeholder={t('account.detail_address')} />
              </div>
            </form>
          )}
        </div>
        ...
        <div className="p-6 bg-surface-container-low flex justify-end gap-4">
          {!isAddingNew ? (
            <>
              <button onClick={onClose} className="px-6 py-2 font-button-text text-on-surface-variant hover:text-secondary transition-colors">{t('common.cancel')}</button>
              <button 
            ...
            <>
              {addresses.length > 0 && (
                <button onClick={() => setIsAddingNew(false)} className="px-6 py-2 font-button-text text-on-surface-variant hover:text-secondary transition-colors">{t('common.back')}</button>
              )}
              <button 
                type="submit" 
