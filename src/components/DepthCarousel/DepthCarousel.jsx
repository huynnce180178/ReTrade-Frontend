...
const DepthCarousel = ({
  ...
  const layout = useCallback(pos => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const tz = -cfg.depth * Math.abs(d);
      const tx = dir * cfg.spread * d;
      const ry = dir * cfg.tilt * clamp(d, -1, 1);

      let opacity = 1;
      if (az > cfg.visibleCards - 1) {
        opacity = clamp(cfg.visibleCards - az, 0, 1);
      }
      if (!shown) opacity = 0;
    ...
    index => {
      ...
      if (active === index) {
        if (data[index]?.productId) {
          navigate(`/product/${data[index].productId}`);
        }
      } else {
        setFocus(index, true);
      }
    },
    [setFocus, active, data, navigate]
  );
    ...
    >
      <div className="depth-carousel__stage" ref={stageRef}>
        {data.map((item, i) => (
          ...
          >
            {item.condition && (
              <span className="depth-carousel__card-condition">{item.condition}</span>
            )}
            ...
            <div className="depth-carousel__card-content">
              {item.seller && (
                <span className="depth-carousel__card-seller">
                  <span className="material-symbols-outlined">storefront</span>
                  {item.seller}
                </span>
              )}
              <h4 className="depth-carousel__card-title">{item.title || item.alt}</h4>
              <div className="depth-carousel__card-footer">
                <span className="depth-carousel__card-price">{item.price}</span>
                {item.productId && (
                  <span className="depth-carousel__card-action-hint">
                    Buy Now
                  </span>
                )}
              </div>
